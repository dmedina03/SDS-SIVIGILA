...
{
    public class PostgradoService : IPostgradoService
    {
        private readonly IPostgradoRepository _repository;
        private readonly IValidator<PostgradoDTO> _validator;

        public PostgradoService(IPostgradoRepository repository, IValidator<PostgradoDTO> validator)
        {
            _repository = repository;
            _validator = validator;
        }

        public async Task<int> AddAsync(PostgradoDTO Dto)
        {
            try
            {
        ...
        public async Task<bool> CreateOrUpdateRangeAsync(IEnumerable<PostgradoDTO> Dtos)
        {
            ...
            foreach (var dto in Dtos)
            {
                ...
                if (!result.IsValid)
                {
                    ...
                    {
                        dataValidationError.Add($"PostgradoDTO[{i}].{element.Key}", element.Value);
        ...
        public async Task<bool> ExistByNameAsync(string nombrePostgrado)
        {
            return await _repository.
                ExistGenericAsync(x => x.NombrePostgrado.ToLower().Trim() == nombrePostgrado.ToLower().Trim());
        }

        public async Task<IEnumerable<PostgradoDTO>> GetAllAsync()
        {
            var data = await _repository.GenericGetAllAsync(x => x.Estado, null, null, x => x);
            return data.MapToDTO();
        }

        public async Task<PostgradoDTO> GetByIdAsync(int Id)
        {
            var data = await _repository.GenericGetAsync(x => new PostgradoDTO
            {
                PostgradoID = x.Id,
            ...
            if (data is null)
                throw new NotFoundException($"No se encontró una Postgrado con el ID {Id}");
            return data;
        }

        public async Task<DataCollection<PostgradoDTO>> GetByParamsAsync(SearchPostgradoDTO Dto)
        {
            return await _repository.GetByParamsAsync(Dto);
        }

        public async Task<bool> UpdateAsync(PostgradoDTO Dto)
        {
        ...
        {
            var data = await _repository.GenericGetAsync(x => new Postgrado
            {
                Id = x.Id,
                Estado = x.Estado
            }, x => x.Id == ID);
        ...
        private void VerifyDuplicades(Dictionary<string, string[]> errors, IEnumerable<PostgradoDTO> lista)
        {
            //Verificamos duplicados por ID
            var dataIds = lista.GroupBy(x => x.PostgradoID).Select(x => new { ID = x.Key, Count = x.Count() })
            ...
            var dataNames = lista.GroupBy(x => x.NombrePostgrado.Trim()).Select(x => new { Name = x.Key, Count = x.Count() })
                        .Where(x => x.Count > 1);
